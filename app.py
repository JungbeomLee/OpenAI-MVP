    ...
    with st.spinner('그림을 생성중입니다..'):
        imgprom = openai.ChatCompletion.create(
            ...
            messages=[
                ...
                {"role": "user",  "content": f'"{story}",Please don\'t say anything other than the story. Keep in mind that the style of the image should be \"Children\'s Book Painting Style\", and the painting should not feel too young or too rough. prompt : '}
                ],
        )
    ...
    with st.spinner('맞춤형 동화를 생성중입니다..'):
        res = openai.ChatCompletion.create(
            ...
            messages=[
                ...
                {"role": "user","content": f" 관심 분야 : {inter},  주제 : {subject}, 장르 : {theme}로 동화를 써줘  반환할 때에는 JSON 형식으로 반환하고, 이야기 외의 다른 말은 하지 말아줘. 내용은 text로 부탁해 이야기 :"}
            ],
        )
